const BasketballTeamScore = ({
  ...
  data: { side: string; status: boolean }
}) => {
  return (
    <div
      className={`flex justify-center items-center ${data.side == 'right' ? 'flex-row-reverse' : ''}`}>
      <div className=" w-[50px] h-[50px] min-w-[30px] min-h-[30px]  sm:size-[90px] lg:size-[120px] rounded-full bg-blue-400 z-10"></div>
      <div className="relative flex flex-col text-[#E9E9E9] font-semibold text-center mx-1 lg:mx-10 sm:mx-3 ">
        ...
        <div
          className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 
                  ...
                  bg-[#173D03] blur-md sm:blur-lg lg:blur-xl ${data.status ? 'hidden' : ''}`}></div>
      </div>
    </div>
  )
}
