...
const Bracket: React.FC = () => {
  ...
  const [finalMatch, setFinalMatch] = useState<Match>({ id: 5, team1: 'KMUTT', team2: 'KMUTNB', score1: 3, score2: 1 });

  const handleScoreChange = (matchId: number, team: 'score1' | 'score2', delta: number, round: 'round1' | 'round2' | 'final') => {
    if (round === 'round1') {
      setRound1Matches(matches => matches.map(match =>
        match.id === matchId ? { ...match, [team]: Math.max(0, match[team] + delta) } : match
      ));
    } else if (round === 'round2') {
      setRound2Matches(matches => matches.map(match =>
        match.id === matchId ? { ...match, [team]: Math.max(0, match[team] + delta) } : match
      ));
    } else {
      setFinalMatch(match => ({ ...match, [team]: Math.max(0, match[team] + delta) }));
    }
  };
    ...
    <div className='brackets-wrapper-container'>
       <div className="sport-section mb-8 d-flex flex-col">
              ...
              <div>
                ...
                <button className ="btn-bracket"onClick={() => setIsAdmin(!isAdmin)}>
  {isAdmin ? "Switch to User" : "Switch to Admin"}
      ...
      <TransformWrapper initialScale={1} minScale={0.5} maxScale={2} wheel={{ wheelEnabled: true }} panning={{ enabled: true, velocityDisabled: true }}>
        <TransformComponent wrapperStyle={{ width: "100%", height: "100vh", cursor: "grab" }}>
                    ...
                    <div className="w-60 bg-black border border-red-600 rounded">
                      <div className="flex justify-between items-center p-3 border-b border-red-700">
                        ...
                        <div className="flex items-center">
                          <button className={isAdmin ? "score-button" : "hidden"} onClick={() => handleScoreChange(match.id, 'score1', -1, 'round1')}>-</button>
                      ...
                      <div className="flex justify-between items-center p-3">
                        ...
                        <div className="flex items-center">
                          <button className={isAdmin ? "score-button" : "hidden"} onClick={() => handleScoreChange(match.id, 'score2', -1, 'round1')}>-</button>
                  ...
                  <div key={match.id} className={`relative match-wrapper ${index === 1 ? 'semifinal-bottom' : ''}`}>
                    <div className="w-60 bg-black border border-red-600 rounded-lg">
                      <div className="flex justify-between items-center p-3 border-b border-red-600">
                        ...
                        <div className="flex items-center">
                          <button className={isAdmin ? "score-button" : "hidden"} onClick={() => handleScoreChange(match.id, 'score1', -1, 'round2')}>-</button>
                          <span className="text-white mx-2">{match.score1}</span>
                          <button className={isAdmin ? "score-button" : "hidden"} onClick={() => handleScoreChange(match.id, 'score1', 1, 'round2')}>+</button>
                        </div>
                      </div>
                      <div className="flex justify-between items-center p-3">
                        ...
                        <div className="flex items-center">
                          <button className={isAdmin ? "score-button" : "hidden"} onClick={() => handleScoreChange(match.id, 'score2', -1, 'round2')}>-</button>
                          <span className="text-white mx-2">{match.score2}</span>
                          <button className={isAdmin ? "score-button" : "hidden"} onClick={() => handleScoreChange(match.id, 'score2', 1, 'round2')}>+</button>
                        </div>
                      </div>
              ...
              <div className="flex flex-col justify-center">
                <div className="w-60 bg-black border border-red-600 rounded">
                  <div className="flex justify-between items-center p-3 border-b border-red-600">
                    ...
                    <div className="flex items-center">
                      <button className={isAdmin ? "score-button" : "hidden"} onClick={() => handleScoreChange(finalMatch.id, 'score1', -1, 'final')}>-</button>
                  ...
                  <div className="flex justify-between items-center p-3">
                    ...
                    <div className="flex items-center">
                      <button className={isAdmin ? "score-button" : "hidden"} onClick={() => handleScoreChange(finalMatch.id, 'score2', -1, 'final')}>-</button>
